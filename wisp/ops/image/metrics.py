...
def clamped_psnr(rgb, gts):
    """Calculate the image PSNR metric after clamping and quantizing to [0,255].

    Assumes the GT image is in [0,1]
    ...
    rgb = (torch.clamp(rgb, 0, 1)*255).to(torch.uint8)
    gts = (torch.clamp(gts, 0, 1)*255).to(torch.uint8)
    mse = torch.mean((rgb[..., :3].float() - gts[..., :3].float()) ** 2).item()
    return 20*np.log10(255.) - 10 * np.log10(mse)

def clamped_mse(rgb, gts):
    """Calculate the image MSE metric after clamping and quantizing to [0,255].
